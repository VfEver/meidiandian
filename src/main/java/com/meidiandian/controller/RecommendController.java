...
public class RecommendController {
	...
	public String recommendGoods(
		...
		if (!StringUtils.isEmpty(id)) {
			...
			//查询出用户和物品之间的具体信息
			List<Map<String, String>> relations = recommendService.findItemUser();
			...
			for (String itemID : user_item.get(user1)) {
				if (!user_item.get(id).contains(itemID)) {
					recommendRate.put(itemID, relationRate.get(id + "," + user1));
				}
			}
			...
			for (String itemID : user_item.get(user2)) {
				if (!user_item.get(id).contains(itemID)) {
					recommendRate.put(itemID, relationRate.get(id + "," + user2));
				}
			}
