...
public class OrderCommentDao implements IOrderCommentDao {
	...
	public void saveOrderComment(OrderComment orderComment) {
		...
		sqlSession.insert(OrderComment.class.getName() + ".saveOrderComment",
				orderComment);
		sqlSession.commit();
		sqlSession.close();
	}
	...
	public void updateOrderCommentStatus(Map<String, String> map) {
		...
		SqlSession sqlSession = sqlSessionFactory.openSession();
		sqlSession.update(OrderComment.class.getName()
				+ ".updateOrderCommentStatus", map);
		sqlSession.commit();
		sqlSession.close();
	}
	...
	public List<OrderComment> findCommentOrder(int userID) {
		...
		SqlSession sqlSession = sqlSessionFactory.openSession();
		List<OrderComment> commentOrderList = sqlSession.selectList(
				OrderComment.class.getName() + ".findCommentOrder", userID);
		sqlSession.close();
